...
> Note: a change has been made so that `e` is no longer a predictable number.  We generate a totally random D, and E is inverse D mod phi.  It checks this in a loop, because the inverse does not always exist; but the probability of finding an inverse is high; so the effect isn't quite noticeable.
